        ...
        <div className="fixed inset-0 bg-[#10182885] flex justify-end items-center h-full z-50">
            <div className="bg-white px-6 pt-[16px] pb-[40px] w-full max-w-[588px] shadow-lg h-full relative">
                ...
                <div className="space-y-4 overflow-y-auto max-h-[80vh]">
                    {['CNIC Number', 'Name', 'Mobile Number'].map((label, index) => (
                        <div key={index}>
                            ...
                            <input type="text" placeholder={label} className="w-full p-2 border border-[#E5E4EA] bg-[#F7F7F9] rounded-[5px] mt-1" disabled={label === 'CNIC Number' || label === 'Mobile Number'} />
                        </div>
                    ))}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        {['Model', 'Quantity'].map((label, index) => (
                            <div key={index}>
                                <label className="block text-[14px] font-medium text-[#4F5B67]">{label}</label>
                                <input type="text" placeholder={label} className="w-full p-2 border border-[#E5E4EA] bg-[#F7F7F9] rounded-[5px] mt-1" disabled />
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="flex items-center gap-4 mt-4 pb-10">
                        ...
                        <div
                            className={`relative w-[36px] h-6 rounded-full transition-colors cursor-pointer ${enabled ? "bg-[#75E0A7]" : "bg-gray-300"}`}
                            onClick={() => setEnabled(!enabled)}
                        >
                            <div
                                className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-all cursor-pointer ${enabled ? "translate-x-5" : "translate-x-0"}`}
                            />
                        </div>
                    </div>
                </div>
                ...
                <div className="flex justify-end shadow absolute bottom-0 w-full right-0 p-5 bg-white gap-4">
                    <button className="flex items-center border px-4 py-2 rounded text-gray-700 hover:bg-gray-100">
                        🖨️ Print Invoice
                    </button>
