						...
						<TableBody className='divide-y divide-gray-100 dark:divide-gray-800 '>
							{orders.map((order) => (
								<TableRow key={order.id} className='w-full mx-3'>
									<TableCell className='py-[26px] px-[10px]'>{order.id}</TableCell>
									<TableCell className='py-[26px] px-[10px]'>{order.customer_name}</TableCell>
									<TableCell className='py-[26px] px-[10px]'>{order.sku}</TableCell>
									<TableCell className='py-[26px] px-[10px]'>{order.purchase_date}</TableCell>
									<TableCell className='py-[26px] px-[10px]'>{order.price}</TableCell>
									<TableCell className='py-[26px] px-[10px]'>
										{order.payment === 'Completed' && (
											<span className='inline-flex items-center px-2.5 py-0.5 justify-center gap-1 rounded-full font-medium text-theme-xs bg-white text-[#079455] border border-[#079455] dark:bg-success-500/15 dark:text-[#079455]'>
												<span className='w-[6px] h-[6px] rounded-full bg-[#079455]'></span> Completed
											</span>
										)}
										{order.payment === 'Due' && (
											<div className='flex flex-col gap-2'>
												<div className='flex gap-2 items-center'>
													<span className='inline-flex items-center px-2.5 py-0.5 justify-center gap-1 rounded-full font-medium text-theme-xs bg-white border border-[#DC6803]  text-[#B54708] dark:bg-warning-500/15 dark:text-[#B54708]'>
														<span className='w-[6px] h-[6px] rounded-full bg-[#DC6803]'></span> Due
													</span>
													<p className='text-[#0BA5EC] decoration-underline text-[12px] cursor-pointer'>Remind</p>
												</div>
												{order.due_date && <div className='text-xs text-[#494949]'>Due date: {order.due_date}</div>}
											</div>
										)}
										{order.payment === 'Pending' && (
											<span className='inline-flex items-center px-2.5 py-0.5 justify-center gap-1 rounded-full font-medium text-theme-xs bg-white   border border-[#DC6803]  text-warning-600 dark:bg-warning-500/15 dark:text-orange-400'>
												<span className='w-[6px] h-[6px] rounded-full bg-[#DC6803]'></span> Pending
											</span>
										)}
									</TableCell>
