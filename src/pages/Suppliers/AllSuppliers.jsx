...
import AddCustomerModal from '../../components/modal/AddSupplierModal';
import EditCustomerModal from '../../components/modal/EditSupplierModal';
import ConfirmDeleteModal from '../../components/modal/DeleteConfirmationModal';
import { generateClient } from 'aws-amplify/api';
import { toast } from 'react-toastify';
import { listSuppliers, supplierByPhone } from '../../graphql/queries';
import { deleteSupplier } from '../../graphql/mutations';
...
export default function Managewarehouses() {
	...
	const searchCustomersByPhone = async (query) => {
		...
		try {
			...
			const data = response.data.supplierByPhone;
	...
	const fetchCustomers = async (token = null) => {
		...
		try {
			...
			const data = response.data.listSuppliers;
			setCustomers(data.items);
			setNextToken(data.nextToken || null);
						...
						<div className='flex justify-end items-center mb-[16px]'>
							<div className='w-[400px]'>
								<label
									...
									Search Supplier By Phone Number
								</label>
